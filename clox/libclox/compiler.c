...
static void initCompiler(Compiler* compiler, FunctionType type)
{
    ...
    // Stack slot 0 is used for `this` in methods, otherwise it's unnamed & innaccessible
...
static void ifStatement()
{
    ...
    emitByte(OP_POP); // drop expression value
    statement();
    int elseJump = emitJump(OP_JUMP);

    patchJump(thenJump);
    emitByte(OP_POP); // if took falsey path, pop here
...
static int resolveLocal(Compiler* compiler, Token* name)
{
    for (int i = compiler->localCount - 1; i >= 0; i--) {
        ...
        if (identifiersEqual(name, &local->name)) {
            if (local->depth == -1) {
                // var has been declared, but not defined. It's not ready for use.
